...
export const WatercolorWashTemplate: React.FC<InvoiceTemplateProps> = ({ invoice, upiLink, qrCodeDataUrl, userPlan }) => {
        ...
        <div className="w-full md:w-2/5 lg:w-1/3 space-y-2 text-sm ml-auto bg-white/60 p-5 rounded-lg shadow-lg backdrop-blur-sm border-t-4 border-pink-400">
          ...
          {(invoice.taxes || []).map(tax => (
            <div key={tax.id} className="flex justify-between">
                <span className="text-slate-600">{tax.name} ({tax.rate}%):</span>
                <span className="text-slate-700 font-semibold">{invoice.currency} {((subtotal * tax.rate) / 100).toFixed(2)}</span>
            </div>
          ))}
