...
const RetroPixelTemplate: React.FC<InvoiceTemplateProps> = ({ invoice, upiLink, qrCodeDataUrl, userPlan }) => {
  ...
  return (
    <div className="p-6 font-[monospace] bg-slate-800 text-lime-300 print:p-0 print:bg-white print:text-black" style={{ fontFamily: pixelFont, imageRendering: 'pixelated' }}>
      <div className="border-4 border-lime-300 p-1 mb-8" style={{ boxShadow: '4px 4px 0px #65a30d, -4px -4px 0px #65a30d, 4px -4px 0px #65a30d, -4px 4px 0px #65a30d' }}>
        <header className="flex justify-between items-center bg-slate-700 p-4 print:bg-slate-100">
          ...
          <div className="text-right">
            <h1 className="text-3xl font-bold uppercase text-red-400 print:text-red-600">{isQuote ? 'QUOTE' : 'INVOICE'}</h1>
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6 text-xs">
        ...
        <div className="bg-slate-700 p-3 border-2 border-red-400 text-left md:text-right print:bg-slate-50 print:border-red-500">
          ...
          <p><strong className="text-yellow-300 print:text-yellow-600">{isQuote ? 'VALID:' : 'DUE:'}</strong> <span className="text-lime-300 print:text-black">{new Date(invoice.dueDate).toLocaleDateString()}</span></p>
        </div>
      </section>
        ...
        <div className="w-full md:w-auto order-last md:order-first mt-4 md:mt-0">
          {!isQuote && (upiLink || qrCodeDataUrl || invoice.manualPaymentLink) && (
            <div className="bg-slate-700 border-2 border-cyan-400 p-3 space-y-2 text-xs text-center md:text-left print:bg-slate-50 print:border-cyan-500">
              ...
              {qrCodeDataUrl && (<div><p className="text-lime-400 mb-1 print:text-lime-700">SCAN QR (UPI):</p><img src={qrCodeDataUrl} alt="UPI QR Code" className="border border-lime-300 p-0.5 bg-white inline-block" style={{width: '70px', height: '70px'}}/></div>)}
              {upiLink && (<div><a href={upiLink} target="_blank" rel="noopener noreferrer" className="inline-block px-3 py-1 bg-red-400 text-slate-800 text-xs font-bold hover:bg-red-300 transition-colors print:bg-red-500 print:text-white">PAY (UPI)</a></div>)}
              {invoice.manualPaymentLink && (<div><a href={invoice.manualPaymentLink} target="_blank" rel="noopener noreferrer" className="inline-block px-3 py-1 bg-cyan-400 text-slate-800 text-xs font-bold hover:bg-cyan-300 transition-colors print:bg-cyan-500 print:text-white">EXTERNAL LINK</a></div>)}
            </div>
          )}
        </div>
      ...
      <section className="mt-6 pt-3 border-t-2 border-dashed border-lime-400 text-xs text-lime-400 print:border-lime-500 print:text-slate-600">
        {invoice.notes && (<div className="mb-2"><h4 className="font-bold text-yellow-300 mb-0.5 print:text-yellow-600">NOTES:</h4><p className="whitespace-pre-wrap">{invoice.notes}</p></div>)}
        {invoice.terms && (<div><h4 className="font-bold text-yellow-300 mb-0.5 print:text-yellow-600">TERMS:</h4><p className="whitespace-pre-wrap">{invoice.terms}</p></div>)}
      </section>
      )}
      ...
      <footer className="text-center text-xs text-lime-500 mt-8 pt-3 border-t-2 border-dashed border-lime-400 print:border-lime-500 print:text-slate-500">
        ...
        {userPlan?.has_branding && (<div className="text-center text-xs text-gray-500 mt-2 print:text-gray-400">Powered by Invoice Maker <span className="text-[0.6rem] opacity-80">by LinkFC</span></div>)}
      </footer>
    </div>
  );
};
