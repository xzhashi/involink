...
export const MonochromeFocusTemplate: React.FC<InvoiceTemplateProps> = ({ invoice, upiLink, qrCodeDataUrl, userPlan }) => {
        ...
        <div className="w-full md:w-2/5 lg:w-1/3 space-y-1 text-sm ml-auto">
          ...
          {(invoice.taxes || []).map(tax => (
            <div key={tax.id} className="flex justify-between p-1">
                <span className="text-gray-600 print:text-black">{tax.name} ({tax.rate}%):</span>
                <span className="text-black font-medium">{invoice.currency} {((subtotal * tax.rate) / 100).toFixed(2)}</span>
            </div>
          ))}
