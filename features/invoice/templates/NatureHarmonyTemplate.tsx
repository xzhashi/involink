...
export const NatureHarmonyTemplate: React.FC<InvoiceTemplateProps> = ({ invoice, upiLink, qrCodeDataUrl, userPlan }) => {
        ...
        <div className="w-full md:w-2/5 lg:w-1/3 space-y-2 text-sm bg-white/60 p-4 rounded-lg shadow-sm backdrop-blur-sm">
          ...
          {(invoice.taxes || []).map(tax => (
            <div key={tax.id} className="flex justify-between">
                <span className="text-emerald-600">{tax.name} ({tax.rate}%):</span>
                <span className="text-emerald-800 font-medium">{invoice.currency} {((subtotal * tax.rate) / 100).toFixed(2)}</span>
            </div>
          ))}
